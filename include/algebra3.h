...
// -    Added Ray, Sample, Color and Material classes
// -    Extended IOStreams
...
enum {
    ...
    MTN,    //refractive index of the material
}; // phong coefficients
...
public:
    ...
    Ray();
    Ray(vec3 & start, vec3 & end, double min_t);
    Ray(const Ray & r);
...
public:
    ...
    Sample();
    Sample(const double x, const double y);
    Sample(const Sample& v); // copy constructor
...
public:
    ...
    RGB& operator =(const RGB& v); // assignment of a vec3
    RGB& operator +=(const RGB& v); // incrementation by a vec3
    RGB& operator -=(const RGB& v); // decrementation by a vec3
    RGB& operator *=(const double d); // multiplication by a constant
    RGB& operator /=(const double d); // division by a constant
    double& operator [](int i); // indexing
    double operator[](int i) const; // read-only indexing

    // Special functions

    //Clips all the values to this max value. Usuall max = 1
    void clip(double max);
    ...
    //Used to return BMP compatible values
    unsigned char getBMPR(int min, int max);
    unsigned char getBMPG(int min, int max);
    unsigned char getBMPB(int min, int max);

    // friends

    friend RGB operator -(const RGB& v); // -v1
    ...
    friend RGB operator *(const RGB& a, const RGB& b); // pointwise mult
    friend RGB operator /(const RGB& a, const double d); // v1 / 3.0
...
public:
    ...
    Material();
    Material(const float ma, const float ml, const float ms, const float msp, const float msm, const float mr, const float mt, const float mtn);
...
inline Ray::Ray(vec3 & start, vec3 & end, double min_t) {
    _e = vec4(start, 1);
    _d = vec4((end - start), 0);
    _min_t = min_t;
}
...
inline Sample::Sample(const double x, const double y) {
	_x = x;
    _y = y;
}

inline Sample::Sample(const Sample& v) {
    _x = v._x;
    _y = v._y;
}
...
inline double & Sample::x() { return _x; }
inline double & Sample::y() { return _y; }

inline void Sample::setX(const double x) { _x = x;}
inline void Sample::setY(const double y) { _y = y;}
...
inline RGB::RGB(const double r, const double g, const double b) {
    _c[RED] = r;
    _c[GREEN] = g;
    _c[BLUE] = b;
}

inline RGB::RGB(const RGB& c) {
    _c[RED] = c._c[RED];
    _c[GREEN] = c._c[GREEN];
    _c[BLUE] = c._c[BLUE];
}
...
inline RGB::RGB(const RGB& a, const RGB& b, const double t) {
    _c[RED] =   (1.-t) * a._c[RED] +    t * b._c[RED];
    _c[GREEN] = (1.-t) * a._c[GREEN] +  t * b._c[GREEN];
    _c[BLUE] =  (1.-t) * a._c[BLUE] +   t * b._c[BLUE];
}
...
inline RGB& RGB::operator *=(const double d) {
    _c[RED] *= d;
    _c[GREEN] *= d;
    _c[BLUE] *= d;
    return *this;
...
inline void RGB::clip(double max) {
    if (_c[RED] > max)
        _c[RED] = max;
    if (_c[GREEN] > max)
        _c[GREEN] = max;
...
inline void RGB::scaleToMax(double resultingMax) {
    double maxValue = max(_c[RED], max(_c[GREEN], _c[BLUE]));
    _c[RED] = linearScale(0, maxValue, 0, resultingMax, _c[RED]);
    _c[GREEN] = linearScale(0, maxValue, 0, resultingMax, _c[GREEN]);
    _c[BLUE] = linearScale(0, maxValue, 0, resultingMax, _c[BLUE]);

}

inline unsigned char RGB::getBMPR(int min, int max) { return (unsigned char) linearScale(min, max, 0, 255, _c[RED]); }
inline unsigned char RGB::getBMPG(int min, int max) { return (unsigned char) linearScale(min, max, 0, 255, _c[GREEN]); }
inline unsigned char RGB::getBMPB(int min, int max) { return (unsigned char) linearScale(min, max, 0, 255, _c[BLUE]); }

inline float RGB::linearScale(float in_low, float in_high, float out_low, float out_high, float x) {
...
inline RGB operator -(const RGB& a) {
    return RGB(-a._c[RED], -a._c[GREEN], -a._c[BLUE]);
}
...
inline int operator ==(const RGB& a, const RGB& b) {
    return (a._c[RED] == b._c[RED]) && (a._c[GREEN] == b._c[GREEN]) && (a._c[BLUE] == b._c[BLUE]);
}
...
#ifdef ALGEBRA3IOSTREAMS
inline ostream& operator <<(ostream& s, const RGB& v) {
...
inline Material::Material(const float ma, const float ml, const float ms, const float msp, const float msm, const float mr, const float mt, const float mtn) {
    _p[MA] = ma;
    _p[ML] = ml;
    _p[MS] = ms;
    _p[MSP] = msp;
...
inline Material::Material(const Material& m) {
    _p[MA] = m._p[MA];
    _p[ML] = m._p[ML];
    _p[MS] = m._p[MS];
    _p[MSP] = m._p[MSP];
...
inline double Material::getMA() const { return _p[MA]; }
inline double Material::getML() const { return _p[ML]; }
inline double Material::getMS() const { return _p[MS]; }
inline double Material::getMSP() const { return _p[MSP]; }
inline double Material::getMSM() const { return _p[MSM]; }
inline double Material::getMR() const { return _p[MR]; }
inline double Material::getMT() const { return _p[MT]; }
inline double Material::getMTN() const { return _p[MTN]; }
